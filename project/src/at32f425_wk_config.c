...
void wk_system_clock_config(void)
{
  ...
  /* config pll clock resource: hext (8mhz) * 12 = 96mhz sclk.
     usb clock is also taken from this pll (see wk_usb_otgfs1_init()),
     so no hick trim/acc dependency is needed for usb timing. */
...
void wk_periph_clock_config(void)
{
  ...
  /* enable dma1 periph clock (spi1 tx / usart1 tx / usart1 rx) */
...
void wk_nvic_config(void)
{
  ...
  /* dma1_channel1 = spi1 tx complete */
  nvic_irq_enable(DMA1_Channel1_IRQn, 0, 0);
  /* dma1_channel2/3 = usart1 tx complete (channel3/usart1 rx never interrupts) */
  nvic_irq_enable(DMA1_Channel3_2_IRQn, 0, 0);
  /* usart1 idle line (marks end of a variable-length dma rx frame) */
